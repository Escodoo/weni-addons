...
def weni_consumption_find(search, apikey=False):
    ...
    if not apikey:
        _logger.error(
            """[WENI_INTEGRATION] API key for Weni required.\n
                          Save this key in System Parameters with key:
                          weni.api_key_weni, value: <your api key>
    ...
    try:
        ...
        _logger.error(
            "[WENI_INTEGRATION] Cannot contact weni servers. "
            "Please make sure that your Internet connection is up and running."
        )
        return None
    ...
    try:
        if result["projects"]:
            _logger.info(
                "[WENI INTEGRATION] Finished get active users by search: %s", search
            )
    except Exception:
        if "detail" in result:
            ...
            _logger.error(
                "[WENI INTEGRATION] Unable to collect active users by search: %s",
                search,
            )
    return result
...
class ResPartner(models.Model):
    ...
    def _weni_consumption_localize(cls, apikey, weni_id="", after="", before=""):
        search = weni_contract_consumption_query_data(
            weni_id=weni_id, after=after, before=before
        )
    ...
    def weni_consumption_localize(self):
        ...
        for rec in self:
            if rec.weni_id:
                result = rec._weni_consumption_localize(
                    apikey, rec.weni_id, after_date_formated, before_date_formated
                )
                if result and "projects" in result:
                    for item in result["projects"]:
                        uuid = item.get("uuid")
                        contract_line = contract_line.search(
                            [("weni_id", "=", uuid)], limit=1
                        )
                        if contract_line:
                            consumption_line_is_invoiced = False
                            for (
                                consumption_line
                            ) in contract_line.contract_line_consumption_ids:
                                if (
                                    consumption_line.consumption_date
                                    == consumption_date
                                ):
                                    if (
                                        consumption_line.invoice_status
                                        == "to_be_invoice"
                                    ):
                                        consumption_line.unlink()
                                    else:
                                        _logger.error(
                                            "[WENI INTEGRATION] Consumption Line is "
                                            "Invoiced: %s",
                                            consumption_line.contract_line_id.name,
                                        )
