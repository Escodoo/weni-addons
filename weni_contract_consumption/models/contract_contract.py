...
class ContractContract(models.Model):
    ...
    contract_consumption_ids = fields.One2many(
        "contract.line.consumption", "contract_id", "Contract Consumption"
    )

    contract_consumption_count = fields.Integer(
        compute="_compute_contract_consumption", string="# Contract Consumption Count"
    )

    @api.depends("contract_consumption_ids")
    def _compute_contract_consumption(self):
        for rec in self:
            rec.contract_consumption_count = len(rec.contract_consumption_ids)
    ...
    def action_view_contract_consumption(self):
        action = self.env.ref(
            "weni_contract_consumption.contract_line_consumption_act_window"
        ).read()[0]
        if self.contract_consumption_count > 1:
            ...
            action["views"] = [
                (
                    self.env.ref(
                        "weni_contract_consumption.contract_line_consumption_form_view"
                    ).id,
                    "form",
                )
            ]
            action["res_id"] = (
                self.contract_consumption_ids
                and self.contract_consumption_ids.ids[0]
                or False
            )
        return action
