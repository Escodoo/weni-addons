...
class KsDashboardNinjaBoard(models.Model):
    ...
    def ks_prepare_item(self, item):
        try:
            ...
            ks_cumulative_field_ids=[]

            if item['ks_chart_cumulative']==True:
                for ks_cumulative in item['ks_chart_cumulative_field']:
                    ks_cumulative_id = self.env['ir.model.fields'].browse([ks_cumulative])
                    if ks_cumulative_id:
                        ks_cumulative_field_ids.append(ks_cumulative_id.id)
                item['ks_chart_cumulative_field'] = [(6, 0, ks_cumulative_field_ids)]
